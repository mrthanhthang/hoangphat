...
(function($) {
	...
	var defaults = {
		...
		show_randomly: 			false,
		thumbs: 				false,
		animateNumberOut: 		{backgroundColor:'#1947be', color:'#fff'},
		animateNumberOver: 		{backgroundColor:'#73b2fe', color:'#000'},
		animateNumberActive: 	{backgroundColor:'#001b5c', color:'#fff'},
		hideTools: 				false,
		fullscreen: 			false,
		xml: 					false,
		dots: 					false,
	...
	$sk.fn.extend({
		...
		setup: function() 
		{
			...
			else if (self.settings.dots)
			{
				...
				self.box_skitter.find('.info_slide_dots').css({'right':15});
			}
			else 
			{
		...
		loadImages: function () 
		{
			...
			$.each(this.settings.images_links, function(i)
			{
				...
				$(img).load(function () {
					...
					self.box_skitter.html('<p style="color:white;background:black;">Error loading img. One or more img were not found.</p>');
				}).attr('src', self_il[0]);
			});
		}, 
